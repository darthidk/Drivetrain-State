...
impl GlobalState {
    ...
    fn update_motors(&mut self, motors: &mut [MotorState; NUM_MOTOR]) {
        for i in 0..NUM_MOTOR {
            motors[i].update(self.clone());
        }
    }
}

impl MotorState {
    fn update(&mut self, global_state: GlobalState) {
        match global_state {
            GlobalState::Drive{vel: global_vel, angle: global_angle} => {
                if self.turning {
                    *self = MotorState{vel: global_vel, angle: global_angle, turning: self.turning};
                } else {
                    *self = MotorState{vel: global_vel, angle: 0, turning: self.turning};
                }
            }
            GlobalState::CrabWalk{vel: global_vel, angle: global_angle} => {
                *self = MotorState{vel: global_vel, angle: global_angle, turning: self.turning};
            }
        };
    }
...
impl UserState {
    ...
    fn update(
        ...
        match self {
            ...
            Self::Test(step) => {
                motor_state.update(global_state);
